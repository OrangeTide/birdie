...
- `src/birdie/` — the MUD-client app only: `main.c` plus networking, telnet, triggers, profiles, and the scripting VM (`bd_net.*`, `bd_session.*`, `bd_mcp.*` (MCP 2.1 / simpleedit), `bd_vm.*`, ...); links the toolkit via `birdie_gui_ludica`
...
- `scripts/dist-check.sh` — guard that the `make dist` bundle copy-lists cover every toolkit source (run by `dist-check`, which gates `dist` and runs in `make test`)
...
`src/birdie/main.c` is a MUD-client shell on the toolkit: a menu bar, a `BD_TERMINAL` output pane, a `BD_INPUT_LINE`, buttons, and a status bar, running on ludica. The client plumbing behind it is substantial and wired through `bd_session`: TCP networking with async resolve (`bd_net`), telnet option negotiation (`bd_telopt`), MXP plus GMCP/MSDP package handling (`bd_mxp`), the in-band MUD Client Protocol 2.1 with the `dns-org-mud-moo-simpleedit` remote-edit package (`bd_mcp`; hooked into the session line pipeline, it opens a `bd_editor` on server-sent content and sends the edit back on Save), a trigger engine (actions, aliases, prompt, gmcp, timers, events in `bd_trigger`) fed from the session's line pipeline, profile storage with title-schema CSV import/export (`bd_profile`), session log sinks (`bd_log`), and a scripting VM (`bd_vm`, with a Lua backend in `bd_vm_lua` and a null fallback). The UI is filling in: the connect picker and edit-profile form are composed with the `bd_dialog` helper, there is an app-wide Settings dialog (terminal grid + colour scheme, persisted to `settings.csv`), the profile form carries autoreconnect/term-type plus an encoding selector (UTF-8 / ISO-8859-1 / Windows-1252 / CP437, the last for DOS/BBS art) wired to an inbound-byte transcode in `bd_net` (`bd_encoding.*`; a server CHARSET UTF-8 negotiation overrides the fallback), a live trigger editor (Session > Triggers...) lists/adds/removes the session's triggers through the `bd_trigger_*` API and persists GUI-added ones to a per-profile `triggers.csv` kept separate from the hand-written `triggers.lua` (now editable in-app via Session > Edit script..., a `bd_editor` on the profile's `triggers.lua` with Lua syntax highlighting from `bd_syntax` and a `bd_findbar` find/replace bar opened with Ctrl-F, saved back to disk), the connect dialog has CSV export (per-column checkboxes) and import with a collision dialog (overwrite/skip/rename), Edit > Colour picker... composes a `#highlight` colour (hex + SGR readouts), and File > Import profiles... (or the connect dialog's Browse...) opens the OS-integrated file chooser (`bd_filedlg` over the `bd_fs` model) to pick a CSV, while profile export gained a Save As... button that opens the same chooser in save mode. A MUD list can also arrive without the chooser: dropping a `.csv` file onto the window imports it, dropping CSV text (e.g. from a gist) imports it directly, and Ctrl-V into the connect dialog's import field pastes-to-import a title-schema list, all sharing the parse/merge core (`bd_profiles_import_csv` plus the `bd_profiles_count_collisions` / `bd_profiles_merge` policy helpers in `bd_profile`). Text that is not a title-schema list is left unclaimed rather than imported. A dropped spreadsheet selection (HTML table) is a future feature. Modal dialogs stack (a chooser opens over another dialog; `bd_modal_open` pushes, `bd_modal_close` pops, the topmost takes input). The `doc/gui/dialogs.md` plan (Phases 0-4) and the `doc/gui/file-dialog.md` plan (P0-P4) are complete. The client core now has some tests (`test_client`, which also covers the profile import merge policies (overwrite/skip/rename) and the CSV-text detection contract behind the drop/paste import; `test_session`; `test_netloop` (which now also covers the `bd_net` TLS handshake + an encrypted round trip against an in-test mbedTLS server); and `test_mcp`), though UI wiring is still untested.

Built: chrome widgets (frame/panel/label/button/menu+pushpins/input-line/terminal), the full v1.0 widget set (`BD_TEXT_FIELD`/`BD_TEXT_AREA`/`BD_LIST`/`BD_SCROLLBAR`/`BD_NOTICE`/`BD_TAB_BAR`), value widgets (slider, knob, toggle, wheel, jog, X-Y pad; all created via `bd_*_desc` structs), form controls for dialogs (`BD_CHECKBOX`, radio group, numeric spinner in `bd_widget_form.*`; a `BD_COMBO` drop-down in `bd_widget_combo.*`; a `bd_dialog` composition helper in `bd_dialog.*` that builds a titled modal with a content column and Enter/Escape-wired button row), an HSV colour picker (`BD_COLORPICK` in `bd_widget_colorpick.*`: SV square + hue bar + swatch + presets, flat-cell gradients, packed RGBA get/set), an explorer/icon-browser widget (icon/list/details view modes, details columns via a model cell hook), a `BD_TREE` expand/collapse hierarchy list (with optional right-aligned dimmed per-node `detail` badges for unread counts/sizes), a `BD_SPLIT` resizable sash container (nestable binary split, horizontal or vertical; ratio-driven panes, a draggable sash as a private widget so pane content stays interactive), a `BD_GROUPBOX` etched-border fieldset and a `BD_SCROLLVIEW` scrolling form container (the latter on a new general `BD_WC_CLIP_CHILDREN` render-clip flag; the widget-class registry is now unbounded/growable), a `BD_TABLE` sortable multi-column data grid (with rich cells: optional per-cell icons and per-row bold/colour/tint via model hooks), a multi-state `BD_INDICATOR` (LED/annunciator), an `bd_actionbar`, a shared icon cell + standalone `BD_ICON` (app-launcher/desktop icon; the dock/actionbar/inventory render and drag through it), a rich-text editor widget (with an autocomplete popup, layered style runs, find/replace highlighting via `bd_editor_find`/`_replace`, and runtime-configurable syntax highlighting via `bd_editor_set_syntax` over the `bd_syntax` state-machine tokenizer: an in-memory text format, built-in Lua/ABC, extension autodetect), plus a composed find/replace bar (`bd_findbar`) over the editor find API (text fields gained `BD_ON_CHANGE_F` on edit + `BD_ON_CLOSE_F` on Escape); both it and `BD_TEXT_AREA` do cross-line stream text selection (drag + Shift+arrows) with clipboard copy/cut, a pressure-sensitive sketch pad (`bd_sketch`), a tab-view container, a NeXTSTEP-style dock, an embedded-WM canvas (`BD_MANAGED_CANVAS`: floating frames + a scoped dock inside a widget, giving primitive MDI on any backend; a per-canvas minimize target selects where minimized frames go via `bd_managed_canvas_set_minimize`: WM desktop icons, an attached dock, or none), 0..1 meters (`bd_meter`: bar/VU/magic-eye/pie/liquid-vial styles with color zones, peak-hold, and ballistics) and a progress bar (`bd_progress`, with a `glass` liquid-tube style matching the vial), a scrolling time-series strip chart (`bd_chart`, xload/system-monitor style), an OS-integrated file chooser (`bd_filedlg` over the UI-agnostic `bd_fs` model: open/save/directory modes, a places sidebar of OS known folders plus mounted volumes, a Recent view from the OS recently-used list, glob filters, a Hidden toggle, and New Folder; the platform seam has a POSIX/freedesktop branch using XDG user-dirs, `getmntent`/`getmntinfo`, and `recently-used.xbel`, plus a compile-verified Win32 branch on `FindFirstFileW`/`SHGetKnownFolderPath`/`GetLogicalDrives`), a top-most overlay primitive (`bd_overlay_*`) for pop-ups that escape their widget rect (the combo list uses it), dwell-triggered tooltips (any widget carrying `BD_TIP_S` shows a non-interactive hover-help bubble, so unlabeled chrome gets a hint), an app-driven context/popup menu (`bd_popmenu` over the overlay primitive; the toolkit never auto-attaches it, so a passthrough canvas keeps control of every click), multiple native windows (GLES backend), Tab focus traversal, clipboard, IME/compose, key-up/repeat, multitouch, pen/tablet input, and OS file drag-and-drop (a host injects a neutral `BD_EV_FILE_DROP` carrying the dropped `paths`; `bd_gui_event` routes it to the extension widget under the point, and otherwise fills a text field there with the path, distinct from the internal cross-widget `BD_EV_DROP`; the ludica backend maps `LUD_EV_DROP` to it). The v0.3 roadmap is complete on the GLES backend. Deferred: Win32/Wayland/macOS backends (tracked in `doc/gui.md`).

The toolkit is at `GUI_VERSION` 0.8.2 (module.mk) heading to a 0.9 release that deliberately batches breaking public-API changes while breakage is still cheap. Landed 0.9 breaks so far: the value-widget `bd_*_desc` create signatures, the `_selected()`/`_select()` selection accessors (from `_active`/`_set_active`), and the unified `bd_managed_canvas_set_minimize`, and a `bd_filter` argument on the backend texture-creation hooks (`load_texture`/`load_texture_mem`/`make_texture`) so a client picks NEAREST vs LINEAR per texture. Prefer API correctness over compatibility until 0.9 is cut.
