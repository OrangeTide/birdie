...
int
bd_utf8_decode(uint32_t *rune, const unsigned char *s, size_t len)
{
	uint32_t v;
	int need;

	if (len == 0) {
		*rune = BD_UTF8_RUNE_ERROR;
		return 0;
	}
...
int
bd_utf8_encode(unsigned char *buf, uint32_t rune)
{
	if (rune <= 0x7F) {
		buf[0] = rune;
		return 1;
...
size_t
bd_utf8_trunc(const char *s, size_t maxbytes)
{
	...
	while (safe < maxbytes && p[safe]) {
		n = bd_utf8_decode(&cp, p + safe, maxbytes - safe);
		if (n <= 0 || cp == BD_UTF8_RUNE_ERROR)
			break;
		safe += (size_t)n;
	}
	return safe;
