...
struct vt_buf {
	...
	struct vt_row	**stash;	/* rows hidden by resize shrink */
	int		stash_len;
	int		stash_cap;
};
...
vt_buf_free(struct vt_buf *buf)
{
	...
	for (i = 0; i < buf->stash_len; i++)
		row_free(buf->stash[i]);
	free(buf->stash);
...
vt_buf_resize(struct vt_buf *buf, int rows, int cols)
{
	...
	/* resize all cell arrays to new column count */
	if (cols != old_cols) {
		for (i = 0; i < old_rows; i++)
			row_resize(buf->grid[i], old_cols, cols);
		for (i = 0; i < buf->stash_len; i++)
			row_resize(buf->stash[i], old_cols, cols);
		for (i = 0; i < buf->ring_cap; i++) {
			if (buf->ring[i])
	...
	if (rows < old_rows) {
		/* shrink: stash excess bottom rows, preserving order */
		int excess = old_rows - rows;
		int need = buf->stash_len + excess;

		if (need > buf->stash_cap) {
			buf->stash_cap = need;
			buf->stash = xrealloc(buf->stash,
			    (size_t)buf->stash_cap *
			    sizeof(buf->stash[0]));
		}

		/* shift existing stash entries right to make room */
		if (buf->stash_len > 0)
			memmove(buf->stash + excess, buf->stash,
			    (size_t)buf->stash_len *
			    sizeof(buf->stash[0]));

		/* prepend the removed grid rows */
		for (i = 0; i < excess; i++)
			buf->stash[i] = buf->grid[rows + i];
		buf->stash_len += excess;

		buf->grid = xrealloc(buf->grid,
		    (size_t)rows * sizeof(buf->grid[0]));
	} else if (rows > old_rows) {
		/* grow: pull rows from stash front, blank-fill rest */
		int delta = rows - old_rows;
		int pull = delta;

		if (pull > buf->stash_len)
			pull = buf->stash_len;

		buf->grid = xrealloc(buf->grid,
		    (size_t)rows * sizeof(buf->grid[0]));

		/* restore stashed rows after current grid end */
		for (i = 0; i < pull; i++)
			buf->grid[old_rows + i] = buf->stash[i];

		/* compact stash */
		buf->stash_len -= pull;
		if (buf->stash_len > 0)
			memmove(buf->stash, buf->stash + pull,
			    (size_t)buf->stash_len *
			    sizeof(buf->stash[0]));

		/* blank-fill any remaining new rows */
		for (i = old_rows + pull; i < rows; i++)
			buf->grid[i] = row_alloc(cols);
	}
